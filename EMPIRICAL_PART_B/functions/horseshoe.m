function [Beta,lambda,tau] = horseshoe(y,X,lambda,tau,sigma_sq,type,ieq)
...
lambda(1) = 1e+10; %Do not restrict intercepts
% lambda(ieq) = 1e+2; %Do not restrict AR(1) terms

% tau = 1; lambda = 1./[1; ones(6,1); 2*ones(6,1); 3*ones(6,1); 4*ones(6,1); 5*ones(6,1)]; %ones(size(lambda));
