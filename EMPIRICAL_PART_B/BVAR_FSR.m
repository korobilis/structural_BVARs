% BVAR_FSR.m
% Code for fast estimation of structural restrictions in Bayesian SVARs. Compared to the baseline case, this model uses
% all 15 variables and identifies at once all seven structural shocks considered in Furlanetto, Ravazzolo, Sarferaz
% (2021).
%
% This code replicates Figure 4 of Section 4.2, as well as Figures C11-C16 of the online supplement, of the paper 
...
tic;
%----------------------------------LOAD DATA----------------------------------------   
data  = xlsread('database.xlsx','full_model');
Y     = data;
names = {'GDP';'prices';'interest rate';'inv/out';'stock prices';'spread';'credit/REV';'EBP';'EBP/VIX';'mortgage rates';...
    'employment';'core prices';'ffr';'stock prices 2';'spread 2'};
[Y,mY,stdY]     = zscore(Y);
...
sg = [ 1     1     1     1     1     1     1;   % VAREQ1
      -1     1     1     1     1     1     1;   % VAREQ2
     NaN     1    -1     1     1     1     1;   % VAREQ3
     NaN    -1   NaN     1     1     1     1;   % VAREQ4
       1   NaN   NaN    -1     1     1     1;   % VAREQ5
     NaN   NaN   NaN   NaN   NaN   NaN   NaN;   % VAREQ6
     NaN   NaN   NaN   NaN    -1     1     1;   % VAREQ7
     NaN   NaN   NaN   NaN   NaN    -1    -1;   % VAREQ8
     NaN   NaN   NaN   NaN   NaN     1    -1;   % VAREQ9
     NaN   NaN   NaN   NaN   NaN   NaN    -1;   % VAREQ10
       1     1     1     1     1     1     1;   % VAREQ11
      -1     1     1     1     1     1     1;   % VAREQ12
     NaN     1    -1     1     1     1     1;   % VAREQ13
       1   NaN   NaN    -1     1     1     1;   % VAREQ14
     NaN   NaN   NaN   NaN   NaN   NaN   NaN];  % VAREQ15
       
sg0 = sg; 
sg0(isnan(sg0)) = 0;
shock_names = {'Agg. Supply';'Agg. Demand';'Monetary';'Investment';'Housing';'Uncertainty';'Credit'};
...
for j = 1:7
    ...
    for i = 1:15
        ...
        %xlabel(['Quarters']);
        %g = yticks;
        %ticks = cell(length(g),1); for i=1:length(g); ticks{i,1} =  (num2str(round(g(i),4)));end
    ...
    pngname = ['LARGEMODEL' num2str(j) '.png'];
    saveas(gcf,pngname);
end
