% BVAR_FSR_1shock.m
% Code for fast estimation of structural restrictions in Bayesian SVARs. Compared to the baseline case we assume only
% r=1 shocks in the system (the financial shock) and no macro shocks are identified.
%
% This code replicates panel (d) of Figure 3 of Section 4.1 of the paper Korobilis, Dimitris (2022), "A new algorithm 
...
for i = 1:6
    subplot(3,2,i)
    shadedplot((1:nhor)',squeeze(FRSlow(i,5,:))',squeeze(FRShigh(i,5,:))',[0.8 0.8 0.8],[0.8 0.8 0.8]) 
    hold all
    plot((1:nhor)',zeros(1,nhor),'r',(1:nhor)',squeeze(FRSmed(i,5,:))','g','LineWidth',3);  
    xlim([1 nhor]); grid on;   
    ax = gca;
    ax.FontSize = 16;
    title(names(i));
...
suptitle(['Responses to a ' str2mat(shock_names) ' shock']);
...
for i = 1:6
    subplot(3,2,i)
    shadedplot((1:nhor)',squeeze(IRFlow(i,1:nhor)),squeeze(IRFhigh(i,1:nhor)),[0.8 0.8 0.8],[0.8 0.8 0.8]) 
    hold all
    plot((1:nhor)',zeros(1,nhor),'r',(1:nhor)',squeeze(IRFmed(i,1:nhor)),'g','LineWidth',3);  
    xlim([1 nhor]); grid on;   
    ax = gca;
    ax.FontSize = 16;
    title(names(i));
...
suptitle(['Responses to a ' str2mat(shock_names) ' shock']);
pngname = ['FACTOR_1shock' num2str(5) '.png'];   
saveas(gcf,pngname);
