% BVAR_FSR_noninf.m
% Code for fast estimation of structural restrictions in Bayesian SVARs. Compared to the baseline case the Horseshoe
% prior is droped and a Uniform prior is used instead, in order to make inference more comparable to the Furlanetto,
% Ravazzolo, Sarferaz (2021) paper (where the authors use a diffusing Uniform prior).
%
% This code replicates Figure C10 of Section C.3.1 of the online supplement of the paper Korobilis, Dimitris (2022), 
...
%1=positive, -1=negative, 0=zero restriction,  NaN=no restriction
      %S1   S2   S3   S4   S5
sg = [  1    1   -1    1    1;    % VAR EQ1
       -1    1   -1    1    1;    % VAR EQ2
      NaN    1    1    1    1;    % VAR EQ3
      NaN   -1  NaN    1    1;    % VAR EQ4
        1  NaN  NaN   -1    1;    % VAR EQ5
      NaN  NaN  NaN  NaN  NaN];   % VAR EQ6
       
sg0 = sg; 
sg0(isnan(sg0)) = 0;
shock_names = {'Supply';'Demand';'Monetary';'Investment';'Financial'};
...
for iter = 1:ngibbs + nburn
    ...
    % STEP 1: update BETA
    for ieq = 1:M
       [beta_i,psi{ieq,1},tau(ieq)] = horseshoe(y_til(:,ieq)./R(ieq,ieq),x./R(ieq,ieq),psi{ieq,1},tau(ieq),1,est_alg,ieq);
       beta_mat(:,ieq) = beta_i;
    end
...
for j = 1:5
    ...
    pngname = ['FACTOR_noninf' num2str(j) '.png'];
    saveas(gcf,pngname);
end
