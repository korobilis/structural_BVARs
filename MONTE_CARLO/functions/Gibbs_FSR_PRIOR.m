function [beta_save,SIGMA_save,L_save,F_save,irf_sign,DIC,DIC2] = Gibbs_FSR_PRIOR(Y,p,lamvar,sg,nhor,ngibbs,nburn,nthin)
...
if M*N < length(cL)+N
    Ltemp = set_lambda(cL,14,3); Ltemp = Ltemp(1:M,1:N); Ltemp(logical(eye(size(Ltemp))))=[]; cL = Ltemp';
elseif M*N > length(cL)+N
    Ltemp = zeros(M*N-N,1); Ltemp(1:39) = cL; cL = Ltemp;
end

lam_prmean  = set_lambda(cL,M,N);
lam_prvar   = lamvar*ones(M,N);
